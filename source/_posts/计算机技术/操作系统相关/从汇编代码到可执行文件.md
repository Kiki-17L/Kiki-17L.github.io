...
> objcopy 是一个“二进制重打包工具”，用于在 ELF、目标文件和裸机可执行格式之间做转换、裁剪

| 指令          | 作用                 |
| ------------- | -------------------- |
| `-O <format>` | 指定输出文件格式     |
| `-j <name>`   | 指定要提取的 section |
...
并且需要自己写一个 `boot.s`，指定`_start`作为入口（`ld` 要识别`_start`，来布局）



gcc 编译 `main.c` 并生成目标代码

```bash
gcc -c main.c -nostartfiles -no-stdlib -ffreestanding -fno-builtin -o main.o
```
...
```bash
ld -T linker.ld -o kernel.elf boot.o main.o
```
