...
$$
\lim_{x\rightarrow a} f(x)=b \Leftrightarrow 取f(x) 定义域内任意数列\{a_n\},\lim_{n\rightarrow\infin}a_n=a,且a_n\ne a,有\lim_{n\rightarrow\infin}f(a_n)=b
$$
...
设 $\displaystyle\lim_{x\rightarrow x_0}f(x)=A$ 

则 $\forall\varepsilon\gt0$ ，$\exist \delta\gt0$ ，当 $0\lt|x-x_0|\lt\delta$ 时，有
$$
|f(x)-A|\lt\varepsilon
$$


又因为 $\displaystyle\lim_{n\rightarrow\infin}x_n=x_0$ ，故对 $\delta\gt0$ ，$\exist N\gt0$ ，当 $n\gt N$ 时，有
$$
|x_n-x_0|\lt\delta
$$


由假设 $x_n\ne x_0$ ，则当 $n\gt N$ 时，有 $0\lt|x_n-x_0|\lt\delta$ ，从而 $|f(x_n)-A|\lt \varepsilon$ ，则
$$
\lim_{n\rightarrow\infin}f(x_n)=A
$$
...
设对任何数列 $\{x_n\} \subset\mathring{U}(x_0;\delta)$ ，且 $\displaystyle\lim_{n\rightarrow\infin}x_n=x_0$ ，则采用	**反证法**

倘若 $x\rightarrow x_0$ 时，函数 $f$ 不以 $A$ 为极限，则存在某一 $\varepsilon_0\gt0$ ，对任何 $\delta \gt0$ (无论多么小)，总存在一点 $x$ ，尽管 $0\lt|x-x_0|\lt\delta$，但有
$$
|f(x)-A|\ge\varepsilon_0
$$
现依次取 $\delta=\delta'_1,\delta'_2,\cdots,\delta'_n,\cdots$ ，则存在相应的点 $x_1,x_2,x_3,\cdots,x_n,\cdots$ ,使得
$$
0\lt|x_n-x_0|\lt\delta'_n
$$
而
$$
...
$$
\begin{aligned}
\lim_{x\rightarrow0}\frac{\ln (1+x)}{x}&=\lim_{x\rightarrow0}\frac{1}{x}\cdot\ln(1+x)\\[10pt]
&=\lim_{x\rightarrow0}\ln (1+x)^{\displaystyle\frac{1}{x}}\\[10pt]
&=\lim_{t\rightarrow e}\ln t\\[10pt]
&=1
\end{aligned}
$$
...
**证**	用 **泰勒公式** 展开 $\tan x=x+\frac{x^3}{3}+o(x^3)$
$$
\begin{align}
\lim_{x\rightarrow0}\frac{\tan x-x}{\frac{1}{3}x^3}&=\lim_{x\rightarrow0}\frac{x+\frac{x^3}{3}+o(x^3)-x}{\frac{1}{3}x^3}\\[10pt]
&=\lim_{x\rightarrow0}\frac{\frac{1}{3}x^3+o(x^3)}{\frac{1}{3}x^3}\\[10pt]
&=\lim_{x\rightarrow0}1+\lim_{x\rightarrow0}\frac{o(x^3)}{\frac{1}{3}x^3}\\[10pt]
&=1+0\\[10pt]
&=1
\end{align}
$$
...
由于 $f(x)$ 在闭区间 $[a,b]$ 上连续，根据 **闭区间上连续函数的最大值最小值定理**

$f(x)$ 在闭区间 $[a,b]$ 上必定取得它的最大值 $M$ 和最小值 $m$，这样只有两种可能的情形：

（1）$M=m$ ，这时 $f(x)$ 在区间上必然取得相同的数值 $M$，即 $f(x)=M$。由此，
$$
\forall x\in(a,b), f'(x)=0\Rightarrow\exists \xi\in(a,b),f'(\xi)=0
$$
（2）$M\gt m$ ，因为 $f(a)=f(b)$ ，所以 $M$ 和 $m$ 这两数中至少有一个不等于 $f(x)$ 在区间端点处的函数值

设 $M\ne f(a)$ ，那么必定在开区间 $(a,b)$ 内有一点 $\xi$ 使 $f(\xi)=M$ 

因此，有 $f(x)\le f(\xi)$ ，由 **费马引理** 可知
$$
f'(\xi)=0
$$
...
**证明**	设函数 $L(x)=f(a)+\displaystyle\frac{f(b)-f(a)}{b-a}(x-a)$

引进	**辅助函数** 
$$
\varphi(x)=f(x)-L(x)
$$
容易验证 $\varphi(x)$ 满足 **罗尔定理** 条件，$\varphi(x)$ 在闭区间 $[a,b]$ 上连续，在开区间 $(a,b)$ 可导，且 $\varphi(a)=\varphi(b)=0$ ，则
...
F(b)-F(a)=F'(\eta)(b-a)
$$
其中 $a\lt\eta\lt b$，根据假定 $F'(\eta)\ne0$，又 $b-a\ne 0$ ，所以
$$
F(b)-F(a)\ne0
$$
 设辅助函数
$$
\varphi(x)=f(x)-\frac{f(a)-f(b)}{F(a)-F(b)}F(x)
$$
显然 $\varphi(x)$ 在闭区间 $[a,b]$ 连续，在开区间 $(a,b)$ 可导，且
$$
\varphi(a)=\varphi(b)=\frac{F(b)f(a)-F(a)f(b)}{F(b)-F(a)}
$$
故 $\varphi(x)$ 适合 **罗尔定理** 条件，因此在 $(a,b)$ 内至少有一点 $\xi$ ，使
$$
...
（3）$\displaystyle\lim_{x\rightarrow a}\frac{f'(x)}{F'(x)}$​ 存在（或为无穷大）
...
因为求 $\displaystyle\frac{f(x)}{F(x)}$ 当 $x\rightarrow a$ 时的极限与 $f(a)$ 及 $F(a)$ 无关，所以假定 $f(a)=F(a)=0$

由条件 （1）、（2）知道，$f(x)$ 及 $F(x)$ 在点 $a$ 的某一邻域内是**连续的**

设 $x$ 是这邻域内的一点，那么在以 $x$ 及 $a$ 为端点的区间上，**柯西中值定理的条件均满足**，因此有
$$
\frac{f(x)}{F(x)}=\frac{f(x)-f(a)}{F(x)-F(a)}=\frac{f'(\xi)}{F'(\xi)}\quad(\xi在x与a之间)
$$
令 $x\rightarrow a$，对上式两端求极限，注意到 $x\rightarrow a$ 时 $\xi \rightarrow a$，再根据条件 （3），便得证
...
$$
此多项式称为 **$f(x)$ 在 $x_0$ 处（或按 $(x-x_0)$ 的幂展开）带有佩亚诺(Peano)余项的 $n$ 阶泰勒公式** ，其中
$$
R_n(x)=o((x-x_0)^n)
$$
...
**证**	记 $R_n(x)=f(x)-P_n(x)$， 则
$$
R_n(x_0)=R_n'(x_0)=R_n''(x_0)=\cdots=R_n^{(n)}(x_0)=0
$$
由于 $f(x)$ 在 $x_0$ 处有 $n$ 阶导数，因此 $f(x)$ 必在 $x_0$ 的某邻域内存在 $(n-1)$ 阶导数，从而 $R(x_0)$ 也在该邻域内 $(n-1)$ 阶可导

反复应用 **洛必达法则**，得
$$
\begin{align}
\lim_{x\rightarrow x_0}\frac{R_n(x)}{(x-x_0)^n}&=\lim_{x\rightarrow x_0}\frac{R_n'(x)}{n(x-x_0)^{n-1}}=\lim_{x\rightarrow x_0}\frac{R_n''(x)}{n(n-1)(x-x_0)^{n-2}}\\[10pt]
&=\cdots=\lim_{x\rightarrow x_0}\frac{R^{(n-1)}_n(x)}{n!(x-x_0)}\\[10pt]
&=\frac{1}{n!}\lim_{x\rightarrow x_0}\frac{R_n^{n-1}(x)-R_n^{(n-1)}(x_0)}{x-x_0}\\[10pt]
&=\frac{1}{n!}R_n^{(n)}(x_0)\\[10pt]
&=0
...
$$
称为 **拉格朗日余项** ，这里 $\xi$ 是 $x_0$ 与 $x$ 之间的某个值
...
（1）如果在 $(a,b)$ 内 $f'(x)\ge0$ ，且等号仅在有限多个点出成立，那么函数 $y=f(x)$ 在 $[a,b]$ 上单调增加

（2）如果在 $(a,b)$ 内 $f'(x)\le0$ ，且等号仅在有限多个点出成立，那么函数 $y=f(x)$ 在 $[a,b]$ 上单调减少
